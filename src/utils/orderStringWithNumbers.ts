...
const BiggerNumberInSequence = (
  dictionaryStringA: Dict[],
  dictionaryStringB: Dict[]
) => {
  for (let i = 0; i < dictionaryStringA.length; i++) {
    if (dictionaryStringA[i].letter > dictionaryStringB[i].letter) {
      return 1;
    }
    if (dictionaryStringA[i].letter < dictionaryStringB[i].letter) {
      return -1;
...
const Compare = (stringA: string, stringB: string) => {
  ...
  for (let pos = 0; pos < biggerStringLength; pos++) {
    ...
    if (IsNumber(arrayCharA[pos]) && IsNumber(arrayCharB[pos])) {
      ...
      if (DictionaryStringA.length > DictionaryStringB.length) {
        return 1;
      }
      if (DictionaryStringA.length < DictionaryStringB.length) {
        return -1;
